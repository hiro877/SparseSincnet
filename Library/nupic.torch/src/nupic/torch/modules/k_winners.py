...
class KWinnersBase(nn.Module, metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        # ブースト関連のパラメータboost_strengthをstate_dictに保存されるようにバッファに入れる。
        # Pythonのfloatのままのコピーを保持し、'if'文でGPUメモリからフェッチするためにブロックせずにその値にアクセスできるようにします。
...
class KWinners1d(KWinnersBase):
    ...
    def __init__(
        ...
        self.register_buffer("duty_cycle", torch.zeros((1, channels, 1)))
    ...
    def forward(self, x):
        ...
        if self.training:
            x = F.kwinners1d(x, self.duty_cycle, self.k,
                             self._cached_boost_strength, self.local,
                             self.break_ties, self.relu, self.inplace)
            self.update_duty_cycle(x)
        else:
            x = F.kwinners1d(x, self.duty_cycle, self.k_inference,
                             self._cached_boost_strength, self.local,
                             self.break_ties, self.relu, self.inplace)
    ...
    def update_duty_cycle(self, x):
        ...
        scale_factor = float(x.shape[2])
        period = min(self.duty_cycle_period, self.learning_iterations)
        self.duty_cycle.mul_(period - batch_size)
        s = x.gt(0).sum(dim=(0, 2), dtype=torch.float) / scale_factor
        self.duty_cycle.reshape(-1).add_(s)
        self.duty_cycle.div_(period)

    def entropy(self):
        entropy = super(KWinners1d, self).entropy()
        return entropy * self.n / self.channels
